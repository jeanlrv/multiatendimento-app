...
export default function DepartmentsPage() {
    ...
    return (
        <div className="space-y-12 max-w-7xl mx-auto relative liquid-glass aurora min-h-0 md:min-h-[calc(100vh-8rem)] pb-12">
            ...
            {/* Modal de Configuração Aero 2.0 */}
            <AnimatePresence>
                {selectedDept && (
                    <DepartmentModal
...
function DepartmentModal({ department, agents, workflows, onClose, onSave }: { department: Department | null, agents: AIAgent[], workflows: WorkflowRule[], onClose: () => void, onSave: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div
                ...
                className="relative w-full max-w-2xl bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh]"
                onClick={e => e.stopPropagation()}
            >
                {/* Modal Header */}
                <div className="p-8 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/2">
                    <div className="flex items-center gap-4">
                        <div
                            className="h-12 w-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg shadow-primary/10"
                            style={{ backgroundColor: selectedColor || '#2563eb' }}
                        >
                            {selectedEmoji}
                        </div>
                        <div>
                            <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic leading-tight">
                    ...
                    <button
                        onClick={onClose}
                        className="h-10 w-10 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-white/5 text-slate-400 hover:text-rose-500 transition-all active:scale-90"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                {/* Form Content */}
                <form onSubmit={handleSubmit(onSubmit)} className="px-8 pb-8 flex-1 overflow-y-auto custom-scrollbar space-y-8">
                    {activeTab === 'geral' && (
                ...
                {/* Modal Footer */}
                <div className="p-6 bg-slate-50/50 dark:bg-white/2 border-t border-slate-100 dark:border-white/5 flex gap-4">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-8 py-3.5 rounded-2xl border border-slate-200 dark:border-white/10 font-black text-[10px] uppercase tracking-widest text-slate-500 hover:bg-rose-500/5 hover:text-rose-500 active:scale-95 transition-all"
                    >
                        Abortar
                    </button>
                    <button
                    ...
                    >
                        {submitting ? <RefreshCcw className="animate-spin h-4 w-4" /> : (
                            <>
                                <Save size={14} />
                                {department ? 'Sincronizar Protocolo' : 'Ativar Unidade'}
                            </>
                        )}
                    </button>
                </div>
