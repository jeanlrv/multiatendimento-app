...
export default function ConnectionsPage() {
    ...
    if (showQr) {
        return (
            <div className="space-y-10 max-w-7xl mx-auto relative liquid-glass aurora min-h-[calc(100dvh-6rem)] md:min-h-[calc(100vh-8rem)] pt-12 pb-12 flex flex-col items-center justify-center">
                <motion.div
...
function ConnectionFormView({ connection, departments, onClose, onSave }: any) {
            ...
            <form onSubmit={handleSave} className="space-y-10">
                ...
                <div className="pt-6 border-t border-slate-100 dark:border-white/5 flex gap-4">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-6 py-4 rounded-xl border border-slate-200 dark:border-white/10 font-bold text-xs uppercase tracking-widest text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
                    >
                        DESCARTAR
                    </button>
                    <button
                        type="submit"
                        disabled={submitting}
                        className="flex-1 bg-primary text-white font-bold py-4 rounded-xl shadow-lg transition-all flex items-center justify-center gap-3 hover:scale-[1.01] active:scale-95 text-sm tracking-widest group"
                    >
                        {submitting ? (
